...
for i in accent_clips[:A_NUM]:
    ...
    for j in audio_clips:
        ...
        if (max_audiosig > mfcc.shape[1]):
            ...
            tot_mfcc=np.concatenate((mfcc,delta_mfcc,delta2_mfcc))
        if(c<4):
            X_train=np.append(X_train,tot_mfcc)
            y_train=np.append(y_train,accent)
            
        else:
            store_acc_name.append(i)
            X_test=np.append(X_test,tot_mfcc)
...
X_train = X_train.reshape(y_train.shape[0],MFCC_NUM*3,max_audiosig,1)
X_test = X_test.reshape(y_test.shape[0],MFCC_NUM*3,max_audiosig,1)
...
def get_model():
    ...
    model.add(tf.keras.layers.Conv2D(32, kernel_size=(2, 2), activation='relu', input_shape=(MFCC_NUM*3,max_audiosig,1)))
